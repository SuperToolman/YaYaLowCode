...
export function ToolCallCard({ activity }: { activity: AgentToolActivity }) {
  return <Accordion className="mx-0 gap-0"><Accordion.Item id={activity.id} className="mx-0"><Accordion.Heading className="mx-0"><Accordion.Trigger className="mx-0 min-w-0 px-0"><Code className="size-4" /><span className="min-w-0 flex-1 truncate text-xs text-muted" title={activity.name}>Tool · {activity.name}</span><Accordion.Indicator className="shrink-0"><ArrowChevronDown /></Accordion.Indicator></Accordion.Trigger></Accordion.Heading><Accordion.Panel className="px-0"><Surface className="rounded-2xl"><div className="divide-y divide-default"><ToolValue label="参数" value={activity.arguments} /><>{activity.result !== undefined ? <ToolValue label="结果" value={activity.result} /> : null}</>{activity.error ? <p role="alert" className="p-3">{json(activity.error)}</p> : null}</div></Surface></Accordion.Panel></Accordion.Item></Accordion>;
}
