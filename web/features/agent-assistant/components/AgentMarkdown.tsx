...
const markdownComponents: Components = {
  h1: ({ children }) => (
    <h1 className="mb-3 mt-6 text-2xl font-semibold leading-tight first:mt-0">
      {children}
    </h1>
  ),
  h2: ({ children }) => (
    <h2 className="mb-2 mt-5 text-xl font-semibold leading-tight first:mt-0">
      {children}
    </h2>
  ),
  h3: ({ children }) => (
  ...
  li: ({ children }) => (
    <li className="pl-1 marker:text-[var(--color-primary)]">{children}</li>
  ),
  blockquote: ({ children }) => (
    <blockquote className="my-4 border-l-2 border-[var(--color-border)] pl-4 text-[var(--color-text-secondary)]">
      {children}
    </blockquote>
  ),
  a: ({ children, href }) => (
    <a
      className="font-medium text-[var(--color-primary)] underline decoration-current/30 underline-offset-4 hover:decoration-current"
      href={href}
      target="_blank"
      rel="noreferrer"
    >
  ...
  table: ({ children }) => (
    <table className="my-4 block w-full overflow-x-auto border-collapse text-left text-sm">
      {children}
    </table>
  ),
  thead: ({ children }) => (
    <thead className="bg-[var(--color-control-soft)]">{children}</thead>
  ),
  th: ({ children }) => (
    <th className="whitespace-nowrap border border-[var(--color-border)] px-3 py-2 font-semibold">
      {children}
    </th>
  ),
  td: ({ children }) => (
    <td className="border border-[var(--color-border)] px-3 py-2 align-top">
      {children}
    </td>
  ),
  pre: ({ children }) => (
    <pre className="my-4 overflow-x-auto rounded-xl bg-[var(--color-bg-subtle)] p-4 text-[13px] leading-6">
      {children}
    </pre>
  ),
    ...
    return (
      <code
        className={
          isBlock
            ? className
            : "rounded bg-[var(--color-control-soft)] px-1.5 py-0.5 font-mono text-[0.9em] text-[var(--color-primary)]"
        }
      >
        {children}
      </code>
...
const compactMarkdownComponents: Components = {
  ...markdownComponents,
  h1: ({ children }) => (
    <h1 className="mb-2 mt-3 text-sm font-semibold first:mt-0">{children}</h1>
  ),
  h2: ({ children }) => (
  ...
  table: ({ children }) => (
    <table className="my-2 block w-full overflow-x-auto border-collapse text-left text-xs">
      {children}
    </table>
  ),
  th: ({ children }) => (
    <th className="whitespace-nowrap border border-[var(--color-border)] px-2 py-1.5 font-semibold">
      {children}
    </th>
  ),
  td: ({ children }) => (
    <td className="border border-[var(--color-border)] px-2 py-1.5 align-top">
      {children}
    </td>
  ),
  pre: ({ children }) => (
    <pre className="my-2 overflow-x-auto rounded-lg bg-[var(--color-bg-subtle)] p-3 text-[11px] leading-5">
      {children}
    </pre>
  ),
};
...
export function normalizeAgentMarkdown(value: string): string {
  ...
  for (const sourceLine of lines) {
    ...
    if (
      ...
      (/^#{1,6}\s/.test(comparable) ||
        /^[-*+]\s/.test(comparable) ||
        /^\d+[.)]\s/.test(comparable))
    )
      continue;
    const listText = comparable
      .replace(/^(?:[-*+]\s+|\d+[.)]\s+)/, "")
      .replace(/[：:，,。；;]+$/g, "");
    if (
      (/^[-*+]\s+/.test(comparable) || /^\d+[.)]\s+/.test(comparable)) &&
      listText.length >= 4
    ) {
      const duplicate = recentListItems.some(
        (item) =>
          item === listText ||
          (item.length >= 10 && listText.startsWith(item)) ||
          (listText.length >= 10 && item.startsWith(listText)),
      );
      ...
      if (rows.length > 1) {
        normalized.push(
          rows
            .map(
              (row, index) =>
                `${index > 0 ? "|" : ""}${row.trim()}${index < rows.length - 1 ? "|" : ""}`,
            )
            .join("\n"),
        );
        continue;
...
export function cleanAgentText(value: string): string {
  ...
  for (let pass = 0; pass < 3; pass += 1) {
    text = text
      .replace(
        /([\u4e00-\u9fffA-Za-z0-9【】「」\[\]（）()、：:，,。！？!? ]{2,32})\s+\1/g,
        "$1",
      )
      .replace(
        /([\u4e00-\u9fffA-Za-z0-9【】「」\[\]（）()、：:，,。！？!? ]{4,48})([，,。；;：:]\s*)\1/g,
        "$1$2",
      )
      .replace(/([【「\[][^{\n}]{1,40}[】」\]])\1/g, "$1");
  }
