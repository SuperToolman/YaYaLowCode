...
export function RichTextEditor({
  ...
  const editor = useEditor({
    extensions: [
      ...
      Image.configure({
        allowBase64: false,
        resize: { enabled: true, minWidth: 100, minHeight: 80 },
      }),
    ...
    editorProps: {
      attributes: {
        ...
        class: `${styles["rich-text-editor__content"]} min-h-28 px-3 py-2 text-sm leading-6 text-[var(--color-text-primary)] outline-none`,
      },
    },
    onUpdate: ({ editor: nextEditor }) =>
      onChange(nextEditor.getJSON() as RichTextDocument),
  });
  ...
  useEffect(() => {
    ...
    if (JSON.stringify(editor.getJSON()) !== nextValue) {
      editor.commands.setContent(value ?? EMPTY_DOCUMENT, {
        emitUpdate: false,
      });
    }
  }, [editor, value]);
  ...
  return (
    <div className="flex h-full min-h-32 w-full flex-col overflow-hidden rounded-md border border-[var(--color-border)] bg-[var(--color-bg-surface)]">
      {editable || preview ? (
        <RichTextToolbar disabled={!editable} editor={editor} />
      ) : null}
...
function RichTextToolbar({
  ...
  return (
    <div className="flex shrink-0 items-center gap-1 overflow-x-auto border-b border-[var(--color-border)] px-2 py-1">
      <ToolbarButton
        disabled={disabled}
        label="撤销"
        onClick={() => execute(() => editor.chain().focus().undo().run())}
      >
        <ArrowUturnCcwLeft />
      </ToolbarButton>
      <ToolbarButton
        disabled={disabled}
        label="重做"
        onClick={() => execute(() => editor.chain().focus().redo().run())}
      >
        <ArrowUturnCwRight />
      </ToolbarButton>
      <ToolbarButton
      ...
      <ToolbarSelect
        ...
        onChange={(value) =>
          execute(() => editor.chain().focus().setFontSize(value).run())
        }
      ...
      <ToolbarButton
        ...
        disabled={disabled}
        label="加粗"
        onClick={() => execute(() => editor.chain().focus().toggleBold().run())}
      >
        <Bold />
      </ToolbarButton>
      <ToolbarButton
      ...
      <ColorInput
        ...
        onChange={(value) =>
          execute(() => editor.chain().focus().setColor(value).run())
        }
      />
      <ColorInput
        disabled={disabled}
        ...
        onChange={(value) =>
          execute(() => editor.chain().focus().setBackgroundColor(value).run())
        }
      />
      <ToolbarButton
...
function ToolbarButton({
  ...
  return (
    <button
      aria-label={label}
      className={[
        "flex h-7 shrink-0 items-center justify-center rounded px-1.5 text-xs hover:bg-[var(--color-bg-hover)] disabled:cursor-default disabled:opacity-50",
        active
...
function ToolbarSelect({
  ...
  return (
    <select
      aria-label={label}
      className="h-7 shrink-0 rounded border-0 bg-transparent px-1 text-xs text-[var(--color-text-secondary)] outline-none disabled:opacity-50"
      disabled={disabled}
      value={value}
...
function ColorInput({
  ...
  return (
    <label
      aria-label={label}
      className="flex h-7 w-7 shrink-0 cursor-pointer items-center justify-center disabled:cursor-default"
    >
      <input
        aria-label={label}
