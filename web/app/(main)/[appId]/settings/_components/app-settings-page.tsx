...
import { setDefaultNavigationEntry, updateApp } from "../../../../lib/api-client";
import {
  getAppForms,
  getAppNavigation,
  getAppResource,
          ...
          {activeSection === "agent-context" ? (
            <>
              <SettingsPanel title="业务概述" description="说明这个应用服务的业务对象、目标用户和主要业务结果。">
                <TextArea aria-label="业务概述" value={businessContext.businessOverview} onChange={(event) => onBusinessContextChange({ ...businessContext, businessOverview: event.currentTarget.value })} placeholder="例如：用于管理客户报价、审批和订单执行。" className="min-h-28" />
              </SettingsPanel>
              <SettingsPanel title="术语与指标口径" description="定义关键业务术语、状态含义和统计口径，避免 Agent 按字段名猜测。">
                <TextArea aria-label="术语与指标口径" value={businessContext.terminology} onChange={(event) => onBusinessContextChange({ ...businessContext, terminology: event.currentTarget.value })} placeholder="例如：商机=尚未转为订单的销售机会；成交额=已确认订单的含税金额。" className="min-h-32" />
              </SettingsPanel>
              <SettingsPanel title="业务流程" description="说明表单之间的业务顺序、状态迁移以及需要一起分析的数据。">
                <TextArea aria-label="业务流程" value={businessContext.processDescription} onChange={(event) => onBusinessContextChange({ ...businessContext, processDescription: event.currentTarget.value })} placeholder="例如：线索转商机，商机生成报价，报价审批通过后创建订单。" className="min-h-32" />
              </SettingsPanel>
              <SettingsPanel title="分析约束" description="填写数据分析时必须遵守的范围、例外和解释规则。">
                <TextArea aria-label="分析约束" value={businessContext.analysisGuidance} onChange={(event) => onBusinessContextChange({ ...businessContext, analysisGuidance: event.currentTarget.value })} placeholder="例如：按月统计时仅纳入已完成订单；取消状态不计入成交额。" className="min-h-28" />
              </SettingsPanel>
            </>
          ) : null}
