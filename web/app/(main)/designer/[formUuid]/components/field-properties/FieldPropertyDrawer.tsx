...
import { Button, Checkbox, ListBox, Select, Switch } from "@heroui/react";
...
export function FieldPropertyPanel({
  ...
  const supportsOptions = field.type !== "member" && isChoiceFieldType(field.type);
            ...
            <div className="min-h-0 flex-1 overflow-y-auto">
              <div className="py-0.5">
                <PropertyPanel>
                  ...
                  {field.type !== "subform" && field.type !== "member" ? <PropertyRow label="默认值" align="start">
                    <DefaultValueEditor
                      fields={fields}
                      field={field}
                      onPropsChange={onPropsChange}
...
type IdentityUser = { id: string; displayName: string; jobNumber: string | null; sourceType: string; status: string };
type IdentityRole = { id: string; name: string; sourceType: string; status: string };
...
function MemberProperties({ field, onPropsChange }: { field: PlacedField; onPropsChange: FieldPropsChangeHandler }) {
  ...
  const selectedIds = scope === "roles" ? field.props.memberRoleIds ?? [] : field.props.memberUserIds ?? [];
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      setLoading(true);
      Promise.all([
        fetch("/api/identity/users", { cache: "no-store" }).then(async (response) => (await response.json()) as IdentityResponse<IdentityUser[]>),
        fetch("/api/identity/roles", { cache: "no-store" }).then(async (response) => (await response.json()) as IdentityResponse<IdentityRole[]>),
  ...
  const candidates = useMemo(() => {
    const matchesSource = (item: { sourceType: string }) => item.sourceType === source;
    return scope === "roles"
  ...
  function updateSelected(id: string, selected: boolean) {
    const next = selected ? [...selectedIds, id] : selectedIds.filter((item) => item !== id);
    onPropsChange(field.id, scope === "roles" ? { memberRoleIds: next } : { memberUserIds: next });
  }
    ...
    <>
      <PropertyRow label="组织来源">
        <MemberPropertySelect
          ...
          onChange={(value) => onPropsChange(field.id, { memberOrganizationSource: value as DesignerFieldProps["memberOrganizationSource"], memberRoleIds: [], memberUserIds: [] })}
        />
      </PropertyRow>
      <PropertyRow label="可选范围">
        <MemberPropertySelect
          ariaLabel="可选范围"
          value={scope}
          options={[{ value: "all", label: "全部成员" }, { value: "roles", label: "指定角色" }, { value: "members", label: "指定成员" }]}
          onChange={(value) => onPropsChange(field.id, { memberSelectableScope: value as DesignerFieldProps["memberSelectableScope"] })}
        />
      </PropertyRow>
      {scope !== "all" ? (
        <PropertyRow label="范围设置">
          <Button variant="ghost" onPress={() => setSelectorOpen(true)} className="h-7 min-w-0 flex-1 justify-between border border-[var(--designer-border)] px-2 text-[11px]">
            <span>{scope === "roles" ? "设置角色" : "设置成员"}</span>
            <span className="text-[10px] text-[var(--color-text-secondary)]">已选 {selectedIds.length}</span>
          </Button>
        </PropertyRow>
      ) : null}
      <PropertyRow label="成员格式">
        <MemberPropertySelect
          ariaLabel="成员格式"
          value={format}
          options={[{ value: "name", label: "姓名" }, { value: "nameJobNumber", label: "姓名(工号)" }, { value: "nameUserId", label: "姓名(userId)" }]}
          onChange={(value) => onPropsChange(field.id, { memberDisplayFormat: value as DesignerFieldProps["memberDisplayFormat"] })}
        />
      </PropertyRow>
      <PropertyRow label="允许多选">
        <Switch aria-label="允许多选" isSelected={Boolean(field.props.memberMultiple)} onChange={(memberMultiple) => onPropsChange(field.id, { memberMultiple })} className="justify-end">
          ...
          <Modal.Container placement="center" scroll="inside" size="sm">
            <Modal.Dialog className="theme-menu-surface flex max-h-[72vh] w-[min(520px,92vw)] flex-col overflow-hidden rounded-xl shadow-[var(--shadow-dialog)]">
              <Modal.Header className="border-b border-[var(--designer-border)] px-5 py-4">
                <Modal.Heading className="text-base font-semibold">选择{scope === "roles" ? "角色" : "成员"}</Modal.Heading>
              </Modal.Header>
              <Modal.Body className="min-h-0 flex-1 overflow-y-auto px-5 py-3">
                <p className="mb-3 text-xs text-[var(--color-text-secondary)]">来源：{MEMBER_SOURCES.find((item) => item.value === source)?.label} · 可多选</p>
                {loading ? <div className="py-8 text-center text-sm text-[var(--color-text-secondary)]">正在加载数据…</div> : null}
                {!loading && candidates.length === 0 ? <div className="py-8 text-center text-sm text-[var(--color-text-secondary)]">该组织来源暂无可选{scope === "roles" ? "角色" : "成员"}。</div> : null}
                <div className="space-y-1">
                  {candidates.map((candidate) => {
                    const id = candidate.id;
                    const label = scope === "roles" ? (candidate as IdentityRole).name : (candidate as IdentityUser).displayName;
                    const detail = scope === "roles" ? id : ((candidate as IdentityUser).jobNumber || id);
                    return <Checkbox key={id} isSelected={selectedIds.includes(id)} onChange={(selected) => updateSelected(id, selected)} className="flex w-full items-center gap-3 rounded-lg px-3 py-2 hover:bg-[var(--designer-surface-soft)]"><Checkbox.Control><Checkbox.Indicator /></Checkbox.Control><Checkbox.Content className="min-w-0 flex-1"><span className="block truncate text-sm">{label}</span></Checkbox.Content><span className="max-w-36 truncate text-[10px] text-[var(--color-text-secondary)]">{detail}</span></Checkbox>;
                  })}
                </div>
              </Modal.Body>
              <Modal.Footer className="flex justify-end border-t border-[var(--designer-border)] px-5 py-3"><Button onPress={() => setSelectorOpen(false)}>完成</Button></Modal.Footer>
            </Modal.Dialog>
          </Modal.Container>
        </Modal.Backdrop>
      </Modal>
...
function MemberPropertySelect({
  ...
  return (
    <Select aria-label={ariaLabel} className="min-w-0 flex-1 text-[11px]" selectedKey={value} onSelectionChange={(key: Key | null) => onChange(key === null ? "" : String(key))}>
      ...
      <Select.Popover>
        <ListBox>{options.map((option) => <ListBox.Item key={option.value} id={option.value} textValue={option.label}>{option.label}</ListBox.Item>)}</ListBox>
      </Select.Popover>
    </Select>
  );
}
