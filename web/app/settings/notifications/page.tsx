      ...
      <div className="space-y-4">
        <Card className="border border-[var(--color-border)] bg-[var(--color-bg-surface)] p-5">
          ...
          <div className="mt-6 space-y-5">
            <Switch isSelected={form.inAppEnabled} isDisabled={loading || saving} onChange={(value) => setForm({ ...form, inAppEnabled: value })}><Switch.Content>启用站内流程通知</Switch.Content><Switch.Control><Switch.Thumb /></Switch.Control></Switch>
        ...
        <Card className="border border-[var(--color-border)] bg-[var(--color-bg-surface)] p-5">
          ...
          <div className="mt-6 space-y-5">
            <div className="space-y-3 border-b border-[var(--color-border)] pb-5"><Switch isSelected={form.dingtalkEnabled} isDisabled={loading || saving} onChange={(value) => setForm({ ...form, dingtalkEnabled: value })}><Switch.Content>预启用钉钉通知</Switch.Content><Switch.Control><Switch.Thumb /></Switch.Control></Switch><Field label="钉钉机器人 Webhook"><Input type="url" value={form.dingtalkWebhookUrl} disabled={loading || saving} onChange={(event) => setForm({ ...form, dingtalkWebhookUrl: event.currentTarget.value })} placeholder="https://oapi.dingtalk.com/robot/send?..." /></Field></div>
            <div className="space-y-3 border-b border-[var(--color-border)] pb-5"><Switch isSelected={form.emailEnabled} isDisabled={loading || saving} onChange={(value) => setForm({ ...form, emailEnabled: value })}><Switch.Content>预启用邮件通知</Switch.Content><Switch.Control><Switch.Thumb /></Switch.Control></Switch><Field label="发件人地址"><Input type="email" value={form.emailFromAddress} disabled={loading || saving} onChange={(event) => setForm({ ...form, emailFromAddress: event.currentTarget.value })} placeholder="no-reply@example.com" /></Field></div>
            <Switch isSelected={form.websocketEnabled} isDisabled={loading || saving} onChange={(value) => setForm({ ...form, websocketEnabled: value })}><Switch.Content>预启用 WebSocket 实时推送</Switch.Content><Switch.Control><Switch.Thumb /></Switch.Control></Switch>
          </div>
        </Card>
      </div>
    </SettingsContentCard>
