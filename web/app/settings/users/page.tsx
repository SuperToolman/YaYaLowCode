    ...
    <section className="theme-panel flex h-full min-h-0 flex-col overflow-hidden rounded-[24px] shadow-[var(--shadow-card)]">
      ...
      <div className="relative min-h-0 flex-1">
        ...
        <div className="settings-scroll-area h-full min-h-0 min-w-0 flex-1 overflow-y-scroll overscroll-contain">
          <div className="sticky top-0 z-10 grid min-w-[840px] grid-cols-[minmax(180px,1.15fr)_80px_minmax(150px,1fr)_minmax(150px,1fr)_120px] gap-3 border-b border-[var(--color-border)] bg-[var(--color-bg-surface)] px-4 py-3 text-xs font-semibold text-[var(--color-text-secondary)]">
            <div>用户</div><div>来源</div><div>组织</div><div>角色</div><div>状态</div>
          </div>
          {filteredUsers.map((user) => (
            <Button fullWidth variant="ghost" key={user.id} className={`grid h-auto min-w-[840px] grid-cols-[minmax(180px,1.15fr)_80px_minmax(150px,1fr)_minmax(150px,1fr)_120px] items-center gap-3 rounded-none border-b border-[var(--color-border)] px-4 py-3 text-left ${selectedId === user.id ? "bg-[var(--color-primary-soft)]" : "hover:bg-[var(--color-bg-hover)]"}`} onPress={() => setSelectedId(user.id)}>
              <div className="flex min-w-0 items-center gap-3">
                <UserAvatar user={user} size="sm" />
                <div className="min-w-0"><div className="truncate text-sm font-medium text-[var(--color-text-primary)]">{user.displayName}</div><div className="mt-0.5 truncate text-[11px] text-[var(--color-text-secondary)]">{user.title || user.mobile || user.email || user.jobNumber || "—"}</div></div>
              </div>
              <div><SourceTag source={user.sourceType} /></div>
...
function UserAvatar({ user, size }: { user: UserItem; size: "sm" | "lg" }) {
  return (
    <Avatar size={size} className={size === "sm" ? "h-9 w-9 shrink-0 text-sm" : "h-12 w-12 text-lg"}>
...
function getAvatarFallbackText(displayName: string) {
  ...
  return characters.length > 2 ? characters.slice(-2).join("") : characters.join("") || "?";
}
