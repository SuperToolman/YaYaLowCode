...
function RecordRowActions({
  ...
  const actions: RecordAction[] = [
    { id: "view", label: "查看", icon: <Eye className="h-3.5 w-3.5" />, onAction: onOpenDetail },
  ];

  if (formType === "workflow") {
    actions.push(
      { id: "submit", label: "提交", tone: "primary", isDisabled: submitting || record.data.workflowApprovalStatus !== "saved", onAction: () => onWorkflowAction("submit") },
      { id: "reverse", label: "反审", tone: "warning", isDisabled: submitting || record.data.workflowApprovalStatus !== "approved", onAction: () => onWorkflowAction("reverse") },
      { id: "pause", label: "暂停", isDisabled: submitting || record.data.workflowInstanceStatus !== "running", onAction: onPause },
      { id: "resume", label: "恢复", tone: "primary", isDisabled: submitting || record.data.workflowInstanceStatus !== "paused", onAction: () => onWorkflowAction("resume") },
    );
  }

  if (canDeleteRecord) {
    actions.push({ id: "delete", label: deletingRecordId === record.id ? "删除中..." : "删除", icon: <TrashBin className="h-3.5 w-3.5" />, tone: "danger", isDisabled: deletingRecordId === record.id, onAction: onDelete });
  }
  actions.push({ id: "copy", label: "复制记录数据", icon: <Copy className="h-3.5 w-3.5" />, onAction: onCopy });
  ...
  return <div className="flex w-max items-center gap-1.5">
    {directActions.map((action) => <Button key={action.id} type="button" variant="ghost" isDisabled={action.isDisabled} className={`h-8 gap-1 rounded-md border bg-[var(--color-bg-panel)] px-2.5 text-xs ${toneClass[action.tone ?? "default"]}`} onPress={action.onAction}>{action.icon}{action.label}</Button>)}
    {overflowActions.length > 0 ? <Dropdown>
      <Dropdown.Trigger aria-label="更多记录操作" className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-md border border-[var(--color-border)] bg-[var(--color-bg-panel)] text-[var(--color-text-secondary)]">
        <Ellipsis className="h-4 w-4" />
      </Dropdown.Trigger>
      <Dropdown.Popover>
        <Dropdown.Menu aria-label="更多记录操作">
          {overflowActions.map((action) => <Dropdown.Item key={action.id} id={action.id} isDisabled={action.isDisabled} className={action.tone === "danger" ? "text-[var(--color-danger)]" : undefined} onAction={action.onAction}>{action.label}</Dropdown.Item>)}
        </Dropdown.Menu>
      </Dropdown.Popover>
    </Dropdown> : null}
  </div>;
...
export function RecordsTable({
  ...
  const associationForms = useAssociationFormsQuery(associationFormIds) as Map<string, TableAssociationFormData>;
  ...
  const pageRecords = pagination ? records : records.slice(pageStart, pageStart + pageSize);
          ...
          <div className="ml-auto flex shrink-0 items-center gap-4">
            ...
            <Pagination size="sm" aria-label="数据分页" className="shrink-0">
              <Pagination.Content>
                ...
                <Pagination.Item><Pagination.Next isDisabled={activePage === pageCount} onPress={() => setPage(Math.min(pageCount, activePage + 1))}>下一页</Pagination.Next></Pagination.Item>
              </Pagination.Content>
            </Pagination>
          </div>
        </Table.Footer>
