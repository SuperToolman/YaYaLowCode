...
import {
  useWorkflowActionsQuery,
  useWorkflowCommentsQuery,
...
export function DetailAuxiliaryPanel({
  ...
  const postCommentMutation = usePostWorkflowComment(
    record.formUuid,
    record.id,
  );
      ...
      >
        <Tabs.ListContainer>
          <Tabs.List aria-label="详情辅助信息">
            <Tabs.Tab id="comments" className="px-4 py-2 text-sm">
              评论
              <Tabs.Indicator />
            </Tabs.Tab>
            <Tabs.Tab id="history" className="px-4 py-2 text-sm">
              变更记录
              <Tabs.Indicator />
            </Tabs.Tab>
          </Tabs.List>
          ...
          <div>
            ...
            <div className="flex">
              ...
              <div className="flex justify-end">
                ...
                >
                  {postCommentMutation.isPending ? "发布中..." : "发表评论"}
                </Button>
              </div>
            </div>
          </div>
              ...
              >
                ...
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-sm">
                    ...
                    <span className="rounded border border-[var(--color-border)] px-1.5 py-0.5 text-xs text-[var(--color-text-secondary)]">
                      {change.type}
                    </span>
                  </div>
                  <div className="mt-1 text-xs text-[var(--color-text-secondary)]">
                    变更人：{change.actor} · {formatDateTime(change.time)}
                  </div>
                </div>
              </li>
            ))}
...
function workflowActionLabel(action: string) {
  return (
    (
      { submit: "提交流程", approve: "同意", reject: "拒绝" } as Record<
        string,
        string
      >
    )[action] ?? action
