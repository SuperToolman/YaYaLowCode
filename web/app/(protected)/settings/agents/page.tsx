...
import { createAgent, listAgents, listConfigProfiles, updateAgent } from "@/features/identity-access/api";
...
export default function AgentsPage() {
  ...
  async function load() {
    const [agentsResult, profilesResult] = await Promise.all([listAgents({ responseStyle: "fields" }), listConfigProfiles({ responseStyle: "fields" })]);
    const agentsPayload = agentsResult.data as ApiEnvelope<AgentDefinition[]> | undefined;
    const profilesPayload = profilesResult.data as ApiEnvelope<AgentConfigProfile[]> | undefined;
    if (agentsResult.error || profilesResult.error || agentsPayload?.code !== 0 || profilesPayload?.code !== 0 || !agentsPayload.data || !profilesPayload.data) throw new Error(agentsPayload?.message || profilesPayload?.message || "无法加载机器人");
  ...
  async function save(event: React.FormEvent) {
    ...
    const body = { ...form, isDefault: editing?.isDefault ?? false, scopeType: "platform", scopeRefId: null, systemPrompt: "", pluginIds: [], skillIds: [], knowledgeBaseIds: [] };
    const result = editing
      ? await updateAgent({ path: { id: editing.id }, body, responseStyle: "fields" })
      : await createAgent({ body, responseStyle: "fields" });
    const payload = result.data as ApiEnvelope<AgentDefinition> | undefined;
    if (result.error || payload?.code !== 0 || !payload.data) return setMessage(payload?.message || "保存失败");
  ...
  >
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">{agents.map((agent) => { const profile = profiles.find((item) => item.id === agent.profileId); return <Button key={agent.id} variant="ghost" fullWidth onPress={() => openEdit(agent)} className="group h-auto min-h-36 justify-start rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-surface)] p-4 text-left transition hover:border-[var(--color-primary)] hover:shadow-[var(--shadow-card-hover)]"><span className="min-w-0 flex-1"><span className="flex items-start justify-between gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-md bg-[var(--color-primary-soft)] text-base text-[var(--color-primary)]">机</span><span className={`shrink-0 rounded-md px-2 py-1 text-[10px] ${agent.enabled ? "bg-[var(--color-success-soft)] text-[var(--color-success)]" : "bg-[var(--color-bg-subtle)] text-[var(--color-text-secondary)]"}`}>{agent.enabled ? "已启用" : "已停用"}</span></span><span className="mt-3 block break-words text-base font-semibold">{agent.name}</span><span className="mt-1 line-clamp-2 block min-h-10 text-xs leading-5 text-[var(--color-text-secondary)]">{agent.description || "暂无描述"}</span><span className="mt-3 block truncate text-[11px] text-[var(--color-primary)]">配置文件 · {profile?.name ?? "未绑定"}</span></span></Button>; })}</div>
