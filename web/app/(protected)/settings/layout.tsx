...
export default function SettingsLayout({
  ...
  const [deploymentType, setDeploymentType] = useState<"saas" | "local">(
    "saas",
  );
  useEffect(() => {
    void getLicenseStatus<{ deploymentType?: string }>()
      .then((license) => {
        setDeploymentType(
          license.deploymentType === "local" ? "local" : "saas",
        );
      })
          ...
          <ScrollShadow className="h-full min-h-0 overflow-y-auto">
            <section className="flex min-h-full flex-col gap-4 overflow-hidden">
              {settingsGroups.map((group) => {
                const visibleItems = permissionsReady
                  ? group.items.filter(
                      (item) =>
                        ...
                        return (
                          ...
                          >
                            ...
                            <span
                              className={
                                active
                                  ? "mt-0.5 truncate text-[11px] text-[var(--color-text-on-primary)]/75"
                                  : "mt-0.5 truncate text-[11px] text-[var(--color-text-secondary)]"
                              }
                            >
                              {item.description}
                            </span>
