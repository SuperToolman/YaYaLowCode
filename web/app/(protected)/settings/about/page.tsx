...
export default function AboutPlatformPage() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void getLicenseStatus<LicenseStatus>()
        .then((status) => {
  ...
  return (
    <section className={styles.page}>
      <SettingsContentCard
        title="关于平台"
        subtitle="平台版本与当前授权状态。许可证正文和验证公钥不会在页面中展示。"
      >
        <div className={styles.content}>
          <Surface
            className={`${styles.statusPanel} ${license?.valid ? styles.statusValid : styles.statusInvalid}`}
          >
            <div className={styles.statusCopy}>
              <div className={styles.brandMark}>丫</div>
              <div>
                <p className={styles.product}>丫丫 LowCode</p>
                <p className={styles.version}>版本 0.82a · 平台授权中心</p>
              </div>
            </div>
            <div className={styles.actions}>
              <Chip color={license?.valid ? "success" : "danger"}>
                {loading
                  ? "正在检查"
                  : license?.valid
                    ? "许可证有效"
              ...
              {hasPermission("settings.license") ? (
                <Button onPress={openLicenseManagementModal}>更新签名</Button>
              ) : null}
            </div>
          </Surface>
          ...
          <div className={styles.metrics}>
            <Metric
              label="授权模块"
              value={license?.modules.length ?? 0}
              suffix="项"
            />
            <Metric
              label="平台类型"
              value={license?.deploymentType === "local" ? "本地部署" : "SaaS"}
            />
            <Metric
              label="有效期至"
              value={formatExpiry(license?.expiresAt)}
              compact
            />
          </div>
          ...
          <div className={styles.columns}>
            <Surface className={styles.infoCard}>
              <div className={styles.cardHeader}>
                <h3>授权信息</h3>
                <Chip>{license?.licenseId ? "已登记" : "未读取"}</Chip>
              </div>
            ...
            <Surface className={styles.infoCard}>
              <div className={styles.cardHeader}>
                <h3>平台信息</h3>
                <Chip>{license?.licenseCenterUrl ? "已连接" : "未配置"}</Chip>
              </div>
              <dl className={styles.infoList}>
                <Info
                  label="部署方式"
                  value={
                    license?.deploymentType === "local" ? "本地部署" : "SaaS"
                  }
                />
                <Info
                  label="运营管理平台 API"
                  value={license?.licenseCenterUrl ?? "-"}
                  mono
                />
                <Info
                  label="平台有效期至"
          ...
          <Surface className={styles.modulesCard}>
            <div className={styles.cardHeader}>
              <div>
                <h3>已授权模块</h3>
                <p>模块权限与独立有效期</p>
              </div>
              <Chip>{license?.modules.length ?? 0} 项</Chip>
            </div>
                  ...
                  <div className={styles.module} key={module}>
                    <div>
                      <span className={styles.moduleName}>
                        {license.moduleTitles?.[module] ??
                          defaultModuleTitle(module)}
                      </span>
                      <span className={styles.moduleKey}>{module}</span>
                    </div>
                    <span className={styles.moduleExpiry}>
                      有效期至{" "}
                      {formatExpiry(
                        license.moduleExpiresAt?.[module] ?? license.expiresAt,
                      )}
                    </span>
                  </div>
                ))
              ) : (
                <div className={styles.empty}>暂未授予任何模块</div>
              )}
            </div>
          </Surface>
        </div>
...
function Metric({
  ...
  return (
    <Surface className={styles.metric}>
      ...
      <strong
        className={compact ? styles.metricValueCompact : styles.metricValue}
      >
        {value}
      </strong>
