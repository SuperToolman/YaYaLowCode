...
export default function MessagesPage() {
  ...
  return <PageContentLayout title="消息" subtitle={`${conversations.length} 个会话 · ${realtimeOnline ? "实时连接正常" : "通讯中心"}`} actions={<Tooltip><Tooltip.Trigger><Button isIconOnly aria-label="新建会话" onPress={() => setNewOpen(true)}><Plus /></Button></Tooltip.Trigger><Tooltip.Content>新建会话</Tooltip.Content></Tooltip>}>
    <main className={`${styles.page}`}>
    <div className={styles.shell}>
      <Card className={styles.sidebar}>
        <Card.Header className={styles.header}><div><h2 className={styles.title}>会话</h2><p className={styles.subtitle}>最近的沟通记录</p></div></Card.Header>
        <ScrollShadow className={styles.conversationList}>{conversations.map((item) => { const avatarMember = item.conversationType === "group" ? item.members[0] : item.members.find((member) => member.userId !== user?.id) ?? item.members[0]; return <button key={item.id} type="button" onClick={() => setSelectedId(item.id)} className={`${styles.conversationItem} ${item.id === selectedId ? styles.conversationItemSelected : ""}`}><MyAvatar name={item.title} imageUrl={avatarMember?.avatarUrl} /><span className={styles.conversationSummary}><span className={styles.conversationName}>{item.title}{item.conversationType === "group" ? <span className={styles.conversationType}>群聊</span> : null}</span><span className={styles.conversationPreview}>{item.lastMessagePreview ?? (item.conversationType === "group" ? `${item.members.length} 位成员` : "暂无消息")}</span></span><span className={styles.conversationMeta}>{item.lastMessageAt ? <time className={styles.conversationTime}>{formatConversationTime(item.lastMessageAt)}</time> : null}{item.unreadCount > 0 ? <span className={styles.unread}>{item.unreadCount > 99 ? "99+" : item.unreadCount}</span> : null}</span></button>; })}{conversations.length === 0 ? <Empty text="暂无会话" /> : null}</ScrollShadow>
      </Card>
      <Card className={styles.conversation}>
        <header className={`${styles.header} ${styles.conversationHeader}`}><div><h2 className={styles.conversationTitle}>{selected?.title ?? "选择会话"}</h2><p className={styles.subtitle}>{typingUsers.length ? "对方正在输入…" : selected ? `${realtimeOnline ? "实时连接正常" : "正在重连"}${onlineUsers.length ? ` · ${onlineUsers.length} 人在线` : ""} · ${selected.members.map((member) => member.displayName).join("、")}` : "从左侧选择一条会话"}</p></div>{selected?.conversationType === "group" && selected.members.some((member) => member.userId === user?.id && member.role === "owner") ? <Button onPress={() => setManageOpen(true)}>管理群聊</Button> : null}</header>
        {error ? <div className={styles.error}>{error}</div> : null}
        <ScrollShadow className={styles.messageList}>{selected ? <div className={styles.messageStack}>{nextBeforeSequence ? <Button onPress={() => selectedId && void loadMessages(selectedId, nextBeforeSequence)}>加载更早消息</Button> : null}{messages.map((message) => { const sender = selected.members.find((member) => member.userId === message.senderUserId); const mine = message.senderUserId === user?.id; const myRole = selected.members.find((member) => member.userId === user?.id)?.role; const canRecall = mine && ((currentTime !== null && currentTime - new Date(message.createdAt).getTime() < 2 * 60_000) || (selected.conversationType === "group" && (myRole === "owner" || myRole === "admin"))); return <MessageBubble key={message.id} message={message} mine={mine} canRecall={canRecall} senderName={sender?.displayName ?? (mine ? user?.displayName ?? "当前用户" : "成员")} senderAvatarUrl={sender?.avatarUrl} onRecall={() => void recall(message)} onReedit={() => beginReedit(message)} onRetry={() => { setMessages((current) => current.filter((item) => item.id !== message.id)); void send(message.messageType, message.content, message.fileIds, message.emailSubject ?? undefined); }} />; })}{messages.length === 0 ? <Empty text="还没有消息" /> : null}</div> : <Empty text="没有选中的会话" />}</ScrollShadow>
        <footer className={styles.composerFooter}>{reediting ? <div className={styles.composerNotice}><span>正在重新编辑撤回的消息</span><Button onPress={() => { setReediting(null); setComposer(""); }}>取消</Button></div> : null}<div className={styles.composer}><MentionComposer ref={composerEditor} value={composer} members={selected?.conversationType === "group" ? selected.members : []} disabled={!selected} placeholder={selected ? "输入消息" : "选择会话后输入消息"} onChange={updateComposer} onSend={() => void send(reediting?.messageType ?? "text", composer, reediting?.fileIds ?? [], reediting?.emailSubject ?? undefined)} /><div className={styles.composerToolbar}><div className={styles.toolbarGroup}><Popover><Popover.Trigger><Button isIconOnly aria-label="发送表情" isDisabled={!selected}><FaceSmile /></Button></Popover.Trigger><Popover.Content><Popover.Dialog aria-label="选择表情">{EMOJIS.map((emoji) => <Button key={emoji} isIconOnly onPress={() => composerEditor.current?.insertText(emoji)}>{emoji}</Button>)}</Popover.Dialog></Popover.Content></Popover><ToolButton label="发送文件" disabled={!selected || busy} onPress={() => fileInput.current?.click()}><Paperclip /></ToolButton><ToolButton label="发送邮件" disabled={!selected || busy} onPress={() => setEmailOpen(true)}><Envelope /></ToolButton><input ref={fileInput} type="file" hidden onChange={(event) => { const file = event.target.files?.[0]; if (file) void upload(file); event.currentTarget.value = ""; }} /></div><Tooltip><Tooltip.Trigger><Button isIconOnly aria-label="发送消息" isDisabled={!selected || !composer.trim() || busy} onPress={() => void send(reediting?.messageType ?? "text", composer, reediting?.fileIds ?? [], reediting?.emailSubject ?? undefined)}><PaperPlane /></Button></Tooltip.Trigger><Tooltip.Content>发送消息</Tooltip.Content></Tooltip></div></div></footer>
      </Card>
    </div>
    ...
    <Modal isOpen={emailOpen} onOpenChange={setEmailOpen}><Modal.Backdrop className="theme-modal-backdrop" isDismissable><Modal.Container placement="center" size="md"><Modal.Dialog><Modal.Header><Modal.Heading>发送邮件消息</Modal.Heading><Modal.CloseTrigger aria-label="关闭" /></Modal.Header><Modal.Body><Input autoFocus fullWidth placeholder="邮件主题" value={emailSubject} onChange={(event) => setEmailSubject(event.currentTarget.value)} /><TextArea fullWidth placeholder="邮件正文" value={emailBody} onChange={(event) => setEmailBody(event.currentTarget.value)} /></Modal.Body><Modal.Footer><Button onPress={() => setEmailOpen(false)}>取消</Button><Button isDisabled={!emailSubject.trim() || !emailBody.trim()} onPress={() => { void send("email", emailBody, [], emailSubject); setEmailOpen(false); setEmailBody(""); setEmailSubject(""); }}>发送</Button></Modal.Footer></Modal.Dialog></Modal.Container></Modal.Backdrop></Modal>
    </main>
  </PageContentLayout>;
}
...
const MentionComposer = forwardRef<MentionComposerHandle, { value: string; members: CommunicationConversationResponse["members"]; disabled: boolean; placeholder: string; onChange: (value: string) => void; onSend: () => void }>(function MentionComposer({ value, members, disabled, placeholder, onChange, onSend }, ref) {
  ...
  return <div className={styles.editorWrap}><div ref={editorRef} contentEditable={!disabled} role="textbox" aria-label="消息内容" aria-multiline data-placeholder={placeholder} suppressContentEditableWarning onInput={syncEditor} onKeyDown={(event) => { removePreviousMention(event); if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); onSend(); } }} className={styles.editor} />{showMenu ? <div className={styles.mentionMenu}><div className={styles.mentionHeader}>群成员</div>{query === "" ? <button type="button" className={styles.mentionOption} onMouseDown={(event) => { event.preventDefault(); insertMentions("all"); }}>@所有人</button> : filteredMembers.length > 1 ? <button type="button" className={styles.mentionOption} onMouseDown={(event) => { event.preventDefault(); insertMentions(filteredMembers); }}>@以下成员 <span className={styles.subtitle}>{filteredMembers.length} 人</span></button> : null}<div className={styles.mentionMembers}>{filteredMembers.map((member) => <button key={member.userId} type="button" className={styles.mentionOption} onMouseDown={(event) => { event.preventDefault(); insertMentions([member]); }}><MyAvatar name={member.displayName} imageUrl={member.avatarUrl} /><span>{member.displayName}</span></button>)}</div></div> : null}</div>;
});
...
function MessageBubble({ message, mine, canRecall, senderName, senderAvatarUrl, onRecall, onReedit, onRetry }: { message: UiMessage; mine: boolean; canRecall: boolean; senderName: string; senderAvatarUrl?: string | null; onRecall: () => void; onReedit: () => void; onRetry: () => void }) {
  ...
  if (message.messageType === "system") return <div className={styles.systemMessage}>{message.content}</div>;
  const bubble = <div className={`${styles.messageBubble} ${mine ? styles.messageBubbleMine : ""}`}>{recalled ? <p className={styles.messageText}>消息已撤回</p> : <>{message.messageType === "email" ? <p className={styles.messageText}><Envelope />{message.emailSubject}</p> : null}<p className={styles.messageText}>{message.content}</p>{message.attachments.map((attachment) => attachment.isImage ? <a key={attachment.fileId} href={`/api/files/${attachment.fileId}/download`} target="_blank" className={styles.attachment}><img src={`/api/files/${attachment.fileId}/download`} alt={attachment.name} className={styles.attachmentImage} /><span>{attachment.name} · {formatBytes(attachment.size)}</span></a> : <a key={attachment.fileId} href={`/api/files/${attachment.fileId}/download`} className={styles.attachment}><Paperclip />{attachment.name} · {formatBytes(attachment.size)}</a>)}</>}</div>;
  return <div className={`${styles.messageRow} ${mine ? styles.messageRowMine : ""}`}><div className={`${mine ? styles.messageAvatarMine : ""}`}><MyAvatar name={senderName} imageUrl={senderAvatarUrl} /></div><div className={`${styles.messageContent} ${mine ? styles.messageContentMine : ""}`}><div className={`${styles.messageMeta} ${mine ? styles.messageMetaMine : ""}`}>{mine ? <><time>{formatMessageTime(message.createdAt)}</time><span>{senderName}</span></> : <><span>{senderName}</span><time>{formatMessageTime(message.createdAt)}</time></>}</div><div onContextMenu={(event) => { if (!canOpenActions) return; event.preventDefault(); setMenuPosition({ left: Math.min(event.clientX, window.innerWidth - 128), top: Math.min(event.clientY, window.innerHeight - 96) }); }}>{bubble}</div></div>{menuPosition ? <div ref={menuRef} role="menu" className={styles.contextMenu} style={{ left: menuPosition.left, top: menuPosition.top }} onContextMenu={(event) => event.preventDefault()}>{message.deliveryState === "failed" ? <Button onPress={() => { setMenuPosition(null); onRetry(); }}>重新发送</Button> : null}{recalled ? <Button onPress={() => { setMenuPosition(null); onReedit(); }}>重新编辑</Button> : canRecall ? <Button onPress={() => { setMenuPosition(null); onRecall(); }}>撤回</Button> : null}</div> : null}</div>;
}
...
function ManageGroupModal({ open, conversation, onClose, onSaved }: { open: boolean; conversation: CommunicationConversationResponse; onClose: () => void; onSaved: () => void }) {
  ...
  return <Modal isOpen={open} onOpenChange={(value) => !value && onClose()}><Modal.Backdrop className="theme-modal-backdrop" isDismissable><Modal.Container placement="center" size="md"><Modal.Dialog><Modal.Header><Modal.Heading>管理群聊</Modal.Heading><Modal.CloseTrigger aria-label="关闭" /></Modal.Header><Modal.Body><Input fullWidth value={title} onChange={(event) => setTitle(event.currentTarget.value)} /><div className="max-h-72 space-y-1 overflow-y-auto">{[...conversation.members.map((member) => ({ id: member.userId, displayName: member.displayName })), ...users.filter((item) => !conversation.members.some((member) => member.userId === item.id))].map((item) => <Checkbox key={item.id} isSelected={memberIds.includes(item.id)} onChange={(checked) => setMemberIds((current) => checked ? [...new Set([...current, item.id])] : current.filter((id) => id !== item.id))} className="w-full rounded-md px-2 py-2"><Checkbox.Content><Checkbox.Control><Checkbox.Indicator /></Checkbox.Control>{item.displayName}</Checkbox.Content></Checkbox>)}</div>{error ? <p className="text-xs text-[var(--color-danger)]">{error}</p> : null}</Modal.Body><Modal.Footer><Button onPress={onClose}>取消</Button><Button isDisabled={!title.trim() || memberIds.length < 2} onPress={() => void save()}>保存</Button></Modal.Footer></Modal.Dialog></Modal.Container></Modal.Backdrop></Modal>;
}

function ToolButton({ children, label, disabled, onPress }: { children: React.ReactNode; label: string; disabled: boolean; onPress: () => void }) { return <Tooltip><Tooltip.Trigger><Button isIconOnly aria-label={label} isDisabled={disabled} onPress={onPress}>{children}</Button></Tooltip.Trigger><Tooltip.Content>{label}</Tooltip.Content></Tooltip>; }
function Empty({ text }: { text: string }) { return <div className={styles.empty}><div><Comment /><p>{text}</p></div></div>; }
