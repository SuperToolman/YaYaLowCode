        ...
        match get_user_detail(&client, access_token, &summary.userid).await {
            Ok(mut detail) => {
                if detail.dept_id_list.is_empty() {
                    detail.dept_id_list = summary.dept_id_list.clone();
                }
                if detail.avatar.trim().is_empty() {
                    detail.avatar = summary.avatar.clone();
                }
                detailed_users.push(detail);
            }
            Err(error) => {
                warn!(user_id = %summary.userid, ?error, "dingtalk user detail unavailable; using department-list data");
                detailed_users.push(summary);
            }
        }
    }
