...
一个面向表单设计、数据录入、集成自动化与智能辅助场景的低代码平台原型项目。当前仓库已经包含应用管理、表单设计器、表单运行时、自动化工作流编辑器、身份认证与 RBAC 权限中心、只读 Agent MVP，以及基于 Rust + PostgreSQL 的后端服务。

当前版本：**0.5a**（内部语义化版本：`0.2.0-alpha.0`）
